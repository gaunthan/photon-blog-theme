...
(function() {
    ...
    Markdown.Extra.init = function(converter, options) {
        ...
        converter.hooks.chain("postNormalization",
        function(text) {
        ...
        converter.hooks.chain("postSpanGamut",
        function(text) {
    ...
    Markdown.Extra.prototype.processDefListItems = function(listStr) {
        ...
        listStr = listStr.replace(dt,
        function(match, pre, termsStr) {
...
(function() {
    ...
    function initMarkdownExtra() {
        ...
        function(text) {
            ...
            function(match, mailto, email) {
                return '<a href="mailto:' + email + '">' + email + '</a>';
            });
            // todolist
            text = text.replace(/<li>(<p>)?\[([ xX]?)\] /g,
            function(matched, p, b) {
                p || (p = '');
    ...
    function initToc() {
        ...
        function buildToc(previewContentsElt) {
            ...
            function createAnchor(element) {
                //    var id = element.id || utils.slugify(element.textContent) || 'title1';
        ...
        toc.convert = function(previewContentsElt) {
            ...
            _each(previewContentsElt.getElementsByTagName('p'),
            function(elt) {
    ...
    function initMathJax() {
        ...
        function processMath(i, j, unescape) {
            var block = blocks.slice(i, j + 1).join("").replace(/&/g, "&amp;").replace(/</g, "&lt;").replace(/>/g, "&gt;");
        ...
        preview = null,
        inline = "$",
        blocks, start, end, last, braces, math, HUB = MathJax.Hub;
    ...
    function initUml() {
        ...
        umlDiagrams.convert = function(target, callback) {
            ...
            if (sequenceElems.length > 0 || flowElems.length > 0) {
                if (!_loadUmlJs) {
                    loadJs('http://leanote.com/public/libs/md2html/uml.js',
                    function() {
                        _loadUmlJs = true;
                        convert();
                    });
    ...
    window.md2Html = function(mdText, toElem, callback) {
        ...
        if (mdText.indexOf('$') !== -1) {
            ...
            loadJs("https://cdn.bootcss.com/mathjax/2.6.1/MathJax.js?config=TeX-AMS_HTML",
            function() {
                if (!m) {
                    var m = initMathJax();
                }
